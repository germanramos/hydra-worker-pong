...
# Hydra Worker Pong - Startup script for Hydra Worker Pong

# chkconfig: 35 99 15
# description: Hydra Worker returns the same instances received in the same order.
# processname: hydra-worker-pong
# Default-Start: 2 3 4 5
# Default-Stop: 0 1 6
# config: 
# pidfile: /var/run/hydra-worker-pong.pid
...
APP_NAME=hydra-worker-pong
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
DAEMON=/usr/local/hydra/hydra-worker-pong
DAEMON_ARGS="-config /etc/hydra/hydra-worker-pong.conf"
